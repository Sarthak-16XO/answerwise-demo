...
const AdminReligionChanges = () => {
  ...
  const handleDeleteCities = async () => {
      try {
          ...
          if (cityDataSnap.exists()) {
              const currentCastes = cityDataSnap.data().cityList || [];
              const updatedCities = currentCastes.filter(caste => caste !== deleteCity);
  
              // Update the caste list in Firestore
              await updateDoc(cityDataRef, { casteList: updatedCities });
                ...
                <div className="fixed top-0 left-0 w-full h-full flex justify-center items-center bg-black bg-opacity-50">
                    <div className="bg-white flex-col justify-between items-center w-96 p-3 rounded-lg p-15">
                        ...
                        <div class="mb-4">
                            ...
                            <button type='submit' className='w-full bg-red-300 rounded-lg' onClick={() => handleAddCities()}>Add a Caste</button>
                        </div>
                    </div>
                </div>
            )}
                ...
                <div className="fixed top-0 left-0 w-full h-full flex justify-center items-center bg-black bg-opacity-50">
                    <div className="bg-white flex-col justify-between items-center w-96 p-3 rounded-lg p-15">
                        ...
                        <div class="mb-4">
                            ...
                            <button type='submit' className='w-full text-lg p-3 bg-red-300 rounded-lg' onClick={() => handleDeleteCities()}>Delete a Caste</button>
                        </div>
                    </div>
                </div>
            )}
