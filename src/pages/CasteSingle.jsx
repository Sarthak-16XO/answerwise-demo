...
function CasteIndividual() {
    ...
    useEffect(() => {
        const getAds = async () => {
            ...
            if (adsData.every((ads) => ads.caste !== castename)) {
    ...
    return (
        <>
            <Navbar />
            <div className='w-full bg-slate-50 items-center m-2 lg:m-10 flex flex-row justify-center'>
                <div className='flex flex-col items-center gap-5 '>
                    <h3 className='font-bold text-3xl text-red-700 underline-offset-2'>{castename}</h3>
                    <div className="grid grid-flow-row grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-2">
                        {adsList
                            .filter((ads) => ads.caste === castename)
                            .map((ads) => {
